...
export async function getDashboardData(authUserId: string): Promise<{
  ...
  try {
    const response = await fetch(`${process.env.REPORTING_SERVICE_URL || 'http://localhost:3004'}/reporting/dashboard/${authUserId}`);
    if (!response.ok) throw new Error('Failed to fetch from reporting service');
